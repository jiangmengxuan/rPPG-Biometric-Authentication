...
def my_config():
    ...
    result_dir = 'D:/joint_results/%s' % (train_exp_name)  # store checkpoints and training recording
...
def my_main(_run, total_epoch, batchsize, T, lr, result_dir, fs, pretrain_rppg, num_classes_old, num_classes, reverse):
    ...
    train_list = glob.glob('./data_example/h5_obf/*1.h5')  # train list on pre-exercise videos.
    test_list = glob.glob('./data_example/h5_obf/*.h5')  # inference list on pre-exercise and post-exercises videos.
    ...
    model.load_state_dict(
        torch.load(pretrain_rppg, map_location=device))  # load the rPPG model's weights from the 1st training stage
    ...
    opt = optim.Adam(list(model.parameters()) + list(cls_head.parameters()) + list(ppg_model.parameters()), lr=lr)
    # define the optimizer for cPPG branch
    opt_ppg = optim.Adam(list(ppg_model.parameters()), lr=lr)
    ...
    for e in range(total_epoch):
        epoch_loss = 0
        epoch_acc = 0
        print(f"Epoch {e + 1}/{total_epoch} started.")

        for it in range(
                np.round(180 / (T / fs)).astype('int')):  # 180 means the video length of each video is 180s (3min).
            iteration_loss = 0
            iteration_acc = 0
            print(f"Iteration {it + 1}/{np.round(180 / (T / fs)).astype('int')} started.")

            for batch_idx, (imgs, label) in enumerate(
                    dataloader):  # dataloader randomly samples a video clip with length T
                imgs = imgs.to(device)  # ST map
                label = label.cpu().numpy()  # id label

                ############### rPPG branch training ###############
                _, rppg = model(imgs)
                rppg = reverse * rppg  # reverse rPPG is necessary
                cycle_list = cycle_cut(rppg, fs,
                                       length=90)  # cut the rPPG signal into periodic segments with length 90.
                ...
                ppg_c = torch.tensor(ppg_clip[ppg_choose]).to(
                    device)  # choose cPPG periodic segments from external cPPG datasets
                ...
                epoch_loss += ppg_loss.item()
                epoch_acc += np.mean(np.argmax(ppg_pred_cls.detach().cpu().numpy(), 1) == ppg_l.detach().cpu().numpy())

                print(f"Batch {batch_idx + 1} processed. Loss: {loss.item():.4f}, Acc: {acc:.4f}")

            iteration_loss /= len(dataloader)
            iteration_acc /= len(dataloader)
            print(f"Iteration {it + 1} completed. Average Loss: {iteration_loss:.4f}, Average Acc: {iteration_acc:.4f}")

        epoch_loss /= np.round(180 / (T / fs)).astype('int')
        epoch_acc /= np.round(180 / (T / fs)).astype('int')
        print(f"Epoch {e + 1} completed. Average Loss: {epoch_loss:.4f}, Average Acc: {epoch_acc:.4f}")
