...
avg = 10 # 对连续10个预测片段取平均ID prediction score averaged on consecutive periodic segments
sess = 1 # test on sess 1 (intra-sess testing) or sess 2 (cross-sess testing)
partition = 'test' # 验证集或测试集。使用验证集部分找到最佳的训练轮次，并在最佳训练轮次上使用测试集部分。val or test. using val partition to find the best epoch and use test partition on the best epoch
epoch_list = [27] # list(range(0,3000,100)) # used to find the best epoch in val.
#epoch_list = list(range(1, 31))  # 生成 [1,2,3,...,30] 的列表
...
for e in epoch_list:
    ...
    for sub in range(100): # get the ID prediction scores and GT labels.
        file_pattern = 'D:/code/joint_results/default/1/%d/h5_obf/%03d*%d.np*' % (e, sub + 1, sess)
        results = np.load(glob.glob(file_pattern)[0],
