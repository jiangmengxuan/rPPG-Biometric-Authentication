...
@ex.config
def my_config():
    e = 2900  # the model checkpoint at epoch e
    train_exp_name = 'default'
    train_exp_num = 1  # the training experiment number
    train_exp_dir = 'D:/code/joint_results/%s/%d' % (train_exp_name, train_exp_num)  # training experiment directory
...
def my_main(_run, e, train_exp_dir, device):
    ...
    # Create prediction experiment directory if it does not exist
    if not os.path.exists(pred_exp_dir):  # 修复缩进问题
        os.makedirs(pred_exp_dir)  # 修复缩进问题
    ...
    model.load_state_dict(
        torch.load(train_exp_dir + '/epoch%d_model.pt' % e, map_location=device))  # load weights to the model
    ...
    for h5_path in test_list:
        ...
        with h5py.File(h5_path, 'r') as f:
            ...
            results = {'rppg_list': rppg_list, 'bvp_list': bvp_list, 'bvp_cyc_list': bvp_cyc_list, 'cyc_list': cyc_list,
                       'pred_list': pred_list}
            # Ensure the directory exists before saving the file
            save_path = pred_exp_dir + '/' + h5_path.split('/')[-1][:-3]
